...
export function useRhombus(draw) {
  ...
  function handleMouseMove(e) {
    ...
    const pos = e.target.getStage().getPointerPosition();
    const newState = lodash.cloneDeep(currentSquare);
    const topX = newState.top.x;
    const topY = newState.top.y;
    const bottomX = pos.x;
    const bottomY = pos.y;
    ...
    if (width < height) {
      newState.bottom.x = topX + height;
    } else {
      newState.bottom.y = topY + width;
    }
  ...
  return {
    ...
    setSelected,
    deleteSelected,
    setRhombuses: setSquares,
    deSelectRhombuses:deSelectSquares
  };
}
